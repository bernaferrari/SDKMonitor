...
import com.bernaferrari.sdkmonitor.util.AnimUtils
import java.util.*
    ...
    init {
        ...
        if (a.hasValue(R.styleable.ElasticDragDismissFrameLayout_dragDismissScale)) {
            dragDismissScale = a.getFloat(
                R.styleable.ElasticDragDismissFrameLayout_dragDismissScale, dragDismissScale
            )
        ...
        if (Math.abs(totalDrag) >= dragDismissDistance) { //&& newdrag) {
            ...
            if (mLastActionEvent == MotionEvent.ACTION_DOWN) {
                ...
                animate()
                    ...
                    .setDuration(200L)
                    .setInterpolator(
                        AnimUtils.getFastOutSlowInInterpolator(context)
                    )
                    .setListener(null)
                    .start()
            }
